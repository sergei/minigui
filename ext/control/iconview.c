/*
** $Id: iconview.c 7367 2007-08-16 05:25:35Z xgwang $
**
 ...
 */
static IconviewItem* ivlistItemAdd (IconviewList* pivlist, IconviewItem* preitem, 
                IconviewItem* nextitem, PIVITEMINFO pii, int *idx)
{
    ...
    ret = mglist_add_item ( (MgList *)pivlist, (MgItem *)ci, (MgItem *)preitem, 
...
int iconview_is_in_item (IconviewData* pivdata, int mousex, int mousey, HITEM *phivi)
{
    return isInItem ((MgList *)&pivdata->ivlist, mousex, mousey, (MgItem**)phivi, NULL);
}
                         ...
                         PIVITEMINFO pii, int *idx)
{
    ...
    if ( (pci = ivlistItemAdd (pivlist, (IconviewItem*)prehivi, NULL, pii, &index)) ) {
        int newh = 0;
        if (pivlist->nCol == 1 || mglist_get_item_count((MgList *)pivlist) % pivlist->nCol == 1)
                ...
                HITEM nexthivi, PIVITEMINFO pii, int *idx)
{
    ...
    if ( (pci =ivlistItemAdd (pivlist, (IconviewItem*)prehivi, (IconviewItem*)nexthivi, pii, &index)) ) {
        if (mglist_get_item_count((MgList *)pivlist) % pivlist->nCol == 1)
            mglist_adjust_items_height (hWnd, (MgList *)pivlist, pivscr, pivlist->nItemHeight);
    }
...
int iconview_move_item (IconviewData* pivdata, HITEM hivi, HITEM prehivi)
{
    ...
    mglist_move_item ((MgList *)&pivdata->ivlist, (MgItem *)hivi, (MgItem *)prehivi);
    scrolled_refresh_content (pivscr);
    return 0;
}

int iconview_del_item (HWND hWnd, IconviewData* pivdata, int nItem, HITEM hivi)
{
    IconviewList* pivlist = &pivdata->ivlist;
    IconviewItem* pci;
    ...
    if (hivi)
        ...
        if ( !(pci=(IconviewItem*)mglist_getitem_byindex((MgList *)pivlist, nItem)) )
            return -1;
    }

    if (ivlistItemDel (pivlist, pci) >= 0) {
        int delh = 0;
        if (mglist_get_item_count((MgList *)pivlist) % pivlist->nCol == 0)
...
static void recalc_total_h (HWND hWnd, IconviewData *pivdata)
{
    ...
    if (newh != oldh) {
        mglist_adjust_items_height (hWnd, (MgList *)pivlist, pivscr, newh - oldh);
    }
}

static int iconview_get_item_rect (HWND hWnd, HITEM hivi, RECT *rcItem, BOOL bConv)
{
...
{
    IconviewData* pivdata = (IconviewData*)GetWindowAdditionalData2(hWnd);
    return (int)mglist_is_item_hilight ((MgList *)&pivdata->ivlist, (MgItem *)hivi);
}
...
static void ivDrawItem (HWND hWnd, GHANDLE hivi, HDC hdc, RECT *rcDraw)
{
    ...
    if (iconview_is_item_hilight(hWnd, hivi)) {
        gal_pixel hilight_bkc_color;
        //SetBrushColor (hdc, PIXEL_blue);
        hilight_bkc_color = GetWindowElementColor(BKC_HILIGHT_NORMAL);
        SetBrushColor (hdc, hilight_bkc_color);
        //SetBkColor(hdc, PIXEL_blue);
        SetBkColor (hdc, hilight_bkc_color);
    ...
    if (ivitem->label) {
        ...
        DrawText (hdc, ivitem->label, -1, &rcTxt, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
    }
    ...
    if (ivitem->bmp) {
        ...
        y = rcDraw->top + ( RECTHP(rcDraw) - RECTH(rcTxt) - (int)ivitem->bmp->bmHeight) / 2;
        if (y < rcDraw->top) {
            y = rcDraw->top;
...
static int IconViewCtrlProc (HWND hWnd, int message, WPARAM wParam, LPARAM lParam)
{
    ...
    case MSG_KEYDOWN:
    {
        ...
        curHilighted = (HITEM) mglist_get_hilighted_item((MgList *)&pivdata->ivlist);
        cursel = mglist_get_item_index ((MgList *)&pivdata->ivlist, (MgItem *)curHilighted);

        if (wParam == SCANCODE_CURSORBLOCKDOWN) {
            int next = cursel + pivdata->ivlist.nCol;
            if (GetWindowStyle(hWnd) & IVS_LOOP && next >= count) {
                next -= (count + pivdata->ivlist.nCol - count % pivdata->ivlist.nCol);
                if (next < 0) next += pivdata->ivlist.nCol;
            }
            hivi = (HITEM)mglist_getitem_byindex((MgList *)&pivdata->ivlist, next);
        }
        ...
        if (hivi) {
            if (hivi != curHilighted) {
                mglist_hilight_item ((MgList *)&pivdata->ivlist, (MgItem *)hivi);
                NotifyParentEx (hWnd, GetDlgCtrlID(hWnd), IVN_SELCHANGED, (DWORD)hivi);
            }
