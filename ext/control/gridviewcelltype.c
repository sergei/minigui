/*
** $Id: gridviewcelltype.c 7367 2007-08-16 05:25:35Z xgwang $
**
    ...
    case MSG_ERASEBKGND:
        {
            ...
            editdata->cell->func->get_format(editdata->cell, &format);
...
#if 0
        if(text_color == PIXEL_lightwhite)
...
void GridCellTypeNull_get_format(gvGridCellData* cell, GridCellFormat* format)
{
                                    ...
                                    HDC hdc, int row, int col, RECT* rect)
{
    ...
    cell->func->get_format(cell, &format);
...
void GridCellTypeTableHeader_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
...
void GridCellTypeHeader_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
    format->color_bg = GetWindowElementColor (BKC_BUTTON_DEF);
...
void GridCellTypeText_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
...
void GridCellTypeNumber_set_text_value(gvGridCellData* cell, int row, int col, char* text)
{
    ...
    if (INF[0] != 'I' && INF[0] != 'i')
        ...
        fprintf(stderr, "The inputed number is too big!\n");
}
...
void GridCellTypeNumber_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
    format->color_bg = GetWindowElementColor (BKC_EDIT_DEF);
...
void GridCellTypeCheckBox_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
...
void GridCellTypeSelection_get_format(gvGridCellData* cell, GridCellFormat* format)
{
    memset(format, 0, sizeof(GridCellFormat));
    format->mask = 0;
    format->color_bg = GetWindowElementColor (BKC_CONTROL_DEF);
