/*
** $Id: gif89a.c 7367 2007-08-16 05:25:35Z xgwang $
**
        ...
        while (code >= clear_code) {
            *sp++ = table[1][code];
            if (code == table[0][code]) {
                fprintf (stderr, "MiniGUIExt GIF89a: circular table entry\n");
                return -1;
            }
            code = table[0][code];
        }
...
static int GetCode(MG_RWops *area, int code_size, int flag)
{
    ...
    if ((curbit + code_size) >= lastbit) {
        if (done) {
            if (curbit >= lastbit)
#ifdef _DEBUG
                fprintf (stderr, "MiniGUIExt GIF89a: bad decode\n");
#endif
            return -1;
        }
...
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt GIF89a: GIF version number not 87a or 89a\n");
#endif
...
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt GIF89a: bad screen descriptor\n");
#endif
        return -1;                /* image loading error*/
    }
...
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt GIF89a: have global colormap: %d\n", GifScreen->Background);
#endif
        if (ReadColorMap (area, GifScreen->BitPixel, GifScreen->ColorMap)) {
#ifdef _DEBUG
            fprintf (stderr, "MiniGUIExt GIF89a: bad global colormap\n");
#endif
            return -1;                /* image loading error*/
        }
    }
...
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt GIF89a: bad image size\n");
#endif
        return -1;
    }
...
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt 89a: have local colormap\n");
#endif
        if (ReadColorMap (area, ImageDesc->bitPixel, ImageDesc->ColorMap) < 0) {
#ifdef _DEBUG
            fprintf (stderr, "MiniGUIExt 89a: bad local colormap\n");
#endif
            return -1;
        }
    } else {
...
static int ReadImage (MG_RWops* area, MYBITMAP* bmp, IMAGEDESC* ImageDesc, GIFSCREEN* GifScreen, int ignore)
{
    ...
    if (!ReadOK (area, &c, 1)) {
#ifdef _DEBUG
        fprintf (stderr, "MiniGUIExt GIF89a: eof on image data\n");
#endif
        return -1;
    }
...
#ifdef _DEBUG
        fprintf (stderr,"MiniGUIExt GIF89a: error reading image\n");
#endif
        return -1;
    }
...
#ifdef _DEBUG
fprintf (stderr, "Background: %d, %d, %d.\n",
...
#ifdef _DEBUG
fprintf (stderr, "MiniGUIExt GIF89a: Extension info: %d, %d, %d, %d, %d, %d\n",
...
#ifdef _DEBUG
            fprintf (stderr, "MiniGUIExt GIF89a: Image Descriptor: %d, %d, %d, %d, %d\n",
...
#ifdef _DEBUG
                fprintf (stderr, "MiniGUIExt GIF89a: frame info: %d, %d, %d, %d\n", 
                        frame->off_x, frame->off_y, frame->delay_time, GifScreen.transparent);
#endif

#ifdef _USE_NEWGAL
                if ((frame->mem_dc = CreateMemDCFromMyBitmap (&mybmp, ImageDesc.ColorMap)) == 0) {
#else
                if (ExpandMyBitmap (hdc, &mybmp, ImageDesc.ColorMap, &frame->bmp) < 0) {
#endif
                    free (mybmp.bits);
                    free (frame);
#ifdef _DEBUG
                    fprintf (stderr, "MiniGUIExt GIF89a: Error when expand frame bitmap.\n");
#endif
                    goto error;
                }
#ifdef _USE_NEWGAL
