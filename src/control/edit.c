/*
** $Id: edit.c 9944 2008-04-01 05:27:47Z xwyan $
**
...
static void set_caret_pos (HWND hWnd, PSLEDITDATA sled, int x, BOOL bSel)
{
    ...
    if (x - sled->startx <= 0) {
        ...
        out_chars = GetTextExtentPoint (hdc, (const char*)sled->content.string, 
                                        sled->content.txtlen, 
                                        x - sled->startx,
...
make_charpos_visible (HWND hWnd, PSLEDITDATA sled, int charPos, int *cx)
{
    ...
    if (charPos <= 0)
        ...
        hdc = GetClientDC (hWnd);
        GetTextExtent (hdc, (const char*)sled->content.string, charPos, &txtsize);
        ReleaseDC (hdc);
    }
    if (cx)
...
static void calc_content_width(HWND hWnd, PSLEDITDATA sled, int charPos, int *cx)
{
    SIZE txtsize;
    HDC hdc;
...
static void calc_charpos_cx(HWND hWnd, PSLEDITDATA sled, int charPos, int *cx)
{
    SIZE txtsize;
    HDC hdc;
...
static void set_line_width (HWND hWnd, PSLEDITDATA sled)
{
    ...
    GetTextExtent(hdc, (const char*)(sled->content.string), sled->content.txtlen, &txtsize);
    ReleaseDC (hdc);
...
static int
sleInsertText_refresh (HWND hWnd, PSLEDITDATA sled, char *newtext, int inserting)
{
    int  deleted;
    unsigned char *pIns, *content = sled->content.string;
...
#if 0
static void escenter_del_refresh(HWND hWnd, PSLEDITDATA sled, int del)
{
    RECT left_scroll_rc, right_scroll_rc, refresh_rc;
    int sel_start_x, sel_end_x;
    int old_caret_x, cur_caret_x;
    int scroll_len;
    int old_editpos_x, cur_editpos_x;

    left_scroll_rc.top = sled->rcVis.top;
    left_scroll_rc.bottom = sled->rcVis.bottom;
    right_scroll_rc.top = sled->rcVis.top;
    right_scroll_rc.bottom = sled->rcVis.bottom;
...
static int sleKeyDown (HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case SCANCODE_REMOVE:
    {
        ...
        if (dwStyle & ES_LEFT){
            ...
            sleInsertText (hWnd, sled, NULL, del);
           // escenter_del_refresh(hWnd, sled, del);
        }else{
            sleInsertText (hWnd, sled, NULL, del);
        }
...
static void esright_input_char_refresh(HWND hWnd, PSLEDITDATA sled, char *charBuffer, int chars)
{
    RECT scroll_rc;
    int scroll_len;
...
static void esleft_input_char_refresh(HWND hWnd, PSLEDITDATA sled, char *charBuffer, int chars)
{
        ...
        if (old_sel_start != old_sel_end){
            ...
            if (sel_size >= sled->nVisW){   
                ...
                if (char_size < sel_size){
                    if ((old_nContX > 0) && ((txtlen_x - old_nContX) <= sled->rcVis.right)){//right scroll window 
            ...
            if (old_caretpos_x < sled->rcVis.right){//right scroll window 
                ...
                if ((sled->rcVis.right - old_caretpos_x) < scroll_len){//
                    ...
                    scroll_rc.right = (right < sled->rcVis.right) ? right+1 : sled->rcVis.right; 
                    ScrollWindow(hWnd, scroll_len, 0, &scroll_rc, NULL);
...
{   
    ...
    {
        sled->starty  = sled->topMargin + ( sled->rcVis.bottom - sled->rcVis.top - 
    ...
    case MSG_CHAR:
    {
        ...
        if (wParam == '\b') { //backspace
            ...
            if (sled->editPos == 0 && sled->selStart == sled->selEnd)
                 return 0;
            del = -CHLENPREV((const char *)(sled->content.string), (sled->content.string + sled->editPos) );
    ...
    case MSG_LBUTTONDOWN:
    {
        ...
        if (HISWORD(lParam) < sled->rcVis.top || HISWORD(lParam) > sled->rcVis.bottom)
            break;
    ...
    case EM_GETSEL:
    {
        ...
        strncpy (buffer, (const char*)(sled->content.string + sled->selStart), len); 
