...
BOOL InitSysFont (void)
{
    ...
    for (i = 0; i < NR_SYSLOGFONTS; i++) {
        ...
        if (GetMgEtcIntValue ("systemfont", sys_font_name [i], &font_id) < 0
            ...
            fprintf (stderr, "FONT: Error system logical font identifier: %s.\n", sys_font_name [i]);
            return FALSE;
        }
        id [i] = font_id;
    }
    ...
    for (i = 0; i < nr_fonts; i++) {
        ...
        if (i == 0 || need) {
            ...
            if (!fontGetTypeNameFromName (font_name, type)
                ...
                fprintf (stderr, "FONT: Invalid system logical font name: %s.\n", 
                                font_name);
                goto error_load;
            }
            ...
            if (i == 0 && GetCharsetOps (charset)->bytes_maxlen_char > 1) {
                fprintf (stderr, "FONT: First system font should be a single-byte charset. font_name: %s\n",
                                font_name);
                goto error_load;
            }
...
error_load:
    fprintf (stderr, "FONT: Error in creating system logical fonts!\n");
    for (i = 0; i < nr_fonts; i++) {
        if (sys_fonts [i])
